...
stateQuadsStrip::stateQuadsStrip(AppWindow* ptr) : State(ptr) {
    ...
    choice_color_1->callback([](Fl_Widget* w, void* statePtr){
        ...
        stateQuadsStrip* state = static_cast<stateQuadsStrip*>(statePtr);
        assert(ch != nullptr);
        assert(state != nullptr);
        auto currColor = state->getTetradColor();
        switch (ch->value()){
        ...
        state->setTetradColor(currColor);
        state->callUpdateGraphics(true);
    }, (void*)this);
    ...
    // color edge_4
    auto label_color_4 = new Fl_Box(500 + 30, 266, 200, 30, "Цвет вершины 4:");
    label_color_4->box(FL_NO_BOX);
    label_color_4->align(FL_ALIGN_INSIDE | FL_ALIGN_TOP);
    label_color_4->labelsize(16);
    widgets.push_back(label_color_4);
    // +
    auto choice_color_4 = new Fl_Choice(500 + 40, 286, 180, 25);
    choice_color_4->add("Случайный");
    choice_color_4->add("Красный");
    choice_color_4->add("Зелёный");
    choice_color_4->add("Синий");
    choice_color_4->value(0);
    choice_color_4->callback([](Fl_Widget* w, void* statePtr){
        Fl_Choice* ch = dynamic_cast<Fl_Choice*>(w);
        stateQuadsStrip* state = static_cast<stateQuadsStrip*>(statePtr);
        assert(ch != nullptr);
        assert(state != nullptr);
        auto currColor = state->getTetradColor();
        switch (ch->value()){
            case 0: std::get<3>(currColor) = ElemColor::random; break;
            case 1: std::get<3>(currColor) = ElemColor::red; break;
            case 2: std::get<3>(currColor) = ElemColor::green; break;
            case 3: std::get<3>(currColor) = ElemColor::blue; break;
            default:
                assert("Incorrect value in switch statement!\n" == nullptr);
                break;
        }
    ...
    widgets.push_back(choice_color_4);
...
// -------------------- set --------------------
void stateQuadsStrip::setPointsNumber(size_t newNumber){
    pointsNumber = newNumber;
}

void stateQuadsStrip::setBkgColor(Fl_Color color){
    bkgColor = color;
}
    
void stateQuadsStrip::setTetradColor(TetradColors tripleCol){
    colors = tripleCol;
}


// -------------------- get --------------------
size_t stateQuadsStrip::getPointsNumber() const {
    return pointsNumber;
}

Fl_Color stateQuadsStrip::getBkgColor() const {
    return bkgColor;
}

TetradColors stateQuadsStrip::getTetradColor() const {
    return colors;
}
