...
void GlSubWin::draw() {
    ...
    glBegin(GL_LINE_STRIP);
        for(size_t i = 0; i < pointsNum; ++i) {
            ...
            // glVertex2d(1.0/pointsNum*(i+1)*w(), N(i_i, p, 1.0/pointsNum*(i+1))*h());
        }
    glEnd();
    // }
    glBegin(GL_POINTS);
    ...
    // glBegin(GL_LINE_STRIP);
    // for(size_t i = 0; i < pointsNum; ++i) {
    //     // glm::vec2 p1 = C(p, U[p]+(U[m-p]-U[p])/pointsNum*i);
    //     glm::vec2 p1 = C(p, 1.0/pointsNum*i);
